...
<?php get_header(); ?>
    ...
    <main>
        ...
        <div class="all-projects-title fixed top-1 light text-center roboto-condensed font-size-1-5 font-weight-700 text-uppercase z-index-2">
            <h1><?php the_title(); ?></h1>
        </div>
        ...
        <div class="projects-categories light-background soft-dark relative width-100 padding-top-4 padding-bottom-4 padding-left-3pc padding-right-3pc text-center">
            <?php
                ...
                if ( ! empty( $terms ) && ! is_wp_error( $terms ) ) {
                    $count = count( $terms );
                    $i = 0;
                    $term_list = '<p class="color-hover roboto font-size-0-7 font-weight-300 text-uppercase letter-spacing-0-0-5"><a class="soft-dark" href="' . esc_url( get_permalink() ) . '">' . get_the_title() . ' /&nbsp; </a>';
                    ...
                    echo '<p class="color-hover roboto font-size-0-7 font-weight-300 text-uppercase letter-spacing-0-0-5"><a class="soft-dark" href="' . get_the_title() . '">' . get_the_title() . '</a></p>';
                }
            ?>
        </div>
